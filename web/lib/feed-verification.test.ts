...
void test('readingsMatch: matches across value types and hash 0x/case variation', () => {
  assert.equal(
...
void test('readingsMatch: a value difference is not a match', () => {
  assert.equal(readingsMatch(committed, mismatchingChain), false);
});

void test('readingsMatch: a sourceHash-only difference is not a match either', () => {
  assert.equal(
...
void test('state 1/4 - UNVERIFIED is the initial state', () => {
  assert.equal(initialVerificationState.status, 'UNVERIFIED');
  assert.equal(initialVerificationState.lastVerifiedAt, null);
});

void test('state 1/4 - UNVERIFIED persists across a failure with no prior success', () => {
...
void test('state 2/4 - VERIFIED after a matching check', () => {
...
void test('state 3/4 - LAST_VERIFIED after a prior match is followed by a failed check', () => {
...
void test('state 4/4 - MISMATCH after a completed check that disagrees with the committed file', () => {
...
void test('MISMATCH is never downgraded to LAST_VERIFIED by a later failed check', () => {
  const verified = applyCheckOutcome(initialVerificationState, { kind: 'match', at: 1000 });
...
void test('MISMATCH persists through repeated subsequent failures, not just one', () => {
...
void test('only a fresh successful check can move a row out of MISMATCH', () => {
...
void test('verifyOnce retries exactly once on a thrown error, then succeeds', async () => {
  let calls = 0;
...
void test('verifyOnce gives up as a failure after the retry also fails, not more', async () => {
  let calls = 0;
  const fetchChainReading = async () => {
    calls += 1;
...
void test('verifyOnce returns mismatch on the first successful-but-disagreeing call, without any retry', async () => {
  let calls = 0;
  const fetchChainReading = async () => {
    calls += 1;
...
void test('formatElapsed renders seconds, minutes, and hours sensibly', () => {
