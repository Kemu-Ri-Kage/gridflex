...
live resolve the market's own yesWon() is read back and compared with that
prediction.
...
def read_market(w3: Web3, oracle_contract, address: str, binary_market_abi: Any) -> MarketState:
    market = w3.eth.contract(address=address, abi=binary_market_abi)

    def read(name: str):
        return call_with_retry(getattr(market.functions, name)().call)

    metric_hash = HexBytes(read("metricId"))
    day_key = int(read("dayKey"))
    raw = oracle_contract.functions.getReading(metric_hash, day_key).call()
    ...
    return MarketState(
        ...
        reading_final=bool(oracle_contract.functions.isFinal(metric_hash, day_key).call())
        if published else False,
    )


def list_market_states(w3: Web3, addresses: dict[str, Any]) -> list[MarketState]:
    """Every market MarketFactory has created, read live."""
    ...
    states = []
    for address in factory.functions.getMarkets().call():
        states.append(read_market(w3, oracle, address, binary_market_abi))
...
def chain_timestamp(w3: Web3) -> int:
...
def resolve_market(w3: Web3, account, market_address: str, log_path) -> tuple[str, int, bool]:
    """Send resolve(); return (tx hash, gas cost in wei, yesWon read back)."""
    ...
    yes_won = bool(call_with_retry(market.functions.yesWon().call))
    return tx_hash, gas_cost, yes_won
...
def run(args: argparse.Namespace, summary: Summary) -> int:
    ...
    states = list_market_states(w3, addresses)
    ...
    now = chain_timestamp(w3)
    print(f"GRIDFLEX resolve_markets - {len(states)} market(s), chain time {format_close(now)}\n")
    candidates = []
    for state in states:
    ...
    for state in candidates:
        ...
        tx_hash, gas_cost, yes_won = resolve_market(w3, account, state.address, log_path)
        ...
        if yes_won != predicted:
            raise PublisherError(
                f"{state.address} resolved {outcome}, but its reading predicted "
                f"{'YES' if predicted else 'NO'} - check the oracle reading."
            )
    return 0
