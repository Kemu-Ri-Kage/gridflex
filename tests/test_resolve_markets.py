...
class TestReadMarket(unittest.TestCase):
    ...
    def test_published_final_reading(self):
        ...
        state = rm.read_market(w3, oracle, "0x" + "44" * 20, [])
...
class TestMain(unittest.TestCase):
    def setUp(self):
        ...
        self.w3.eth.get_block.return_value = {"timestamp": NOW}
        ...
        self.patches = [
            ...
            patch.object(rm, "list_market_states", side_effect=lambda w3, a: self.states),
    ...
    def test_an_outcome_that_contradicts_the_reading_stops_the_run(self):
        self.resolve.side_effect = lambda *a: ("0x" + "cd" * 32, 1, False)  # NO, reading says YES
        code, out, err = self.main("--live")
        self.assertEqual(code, 1)
        self.assertEqual(self.resolve.call_count, 1)
        self.assertIn("check the oracle reading", err)
